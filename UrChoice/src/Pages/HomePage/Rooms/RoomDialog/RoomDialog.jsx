...
const RoomDialog = ({ dialogRef, selectedRoom, currentUserId }) => {
    ...
    // Función optimizada con useCallback para verificar el estado de los usuarios
    const checkAllReady = useCallback(() => {
        if (!selectedRoom?.id_room) return;
        
        // Verificamos que haya exactamente 2 usuarios y ambos estén listos
        if (users.length === 2 && users.every(user => user.vote_game === 'LISTO')) {
            startMatch();
        }
    }, [users, selectedRoom]);
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (data.message === 'La sala se ha cerrado correctamente') {
                    ...
                    closeDialog(); // Cierra el diálogo después de iniciar el juego  
                }
            } else {
    ...
    const fetchUsers = async () => {
        ...
        try {
            ...
            if (res.ok && Array.isArray(data)) {
                ...
                // Solo actualizamos si hay cambios para evitar renders innecesarios
    ...
    // Resto del código (handleKickUser, renderUserInfo, etc.) permanece igual
